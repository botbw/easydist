...
device_mesh = DeviceMesh(mesh=torch.arange(4).reshape(2, 2), device_type='cpu')
...
def getDevAnn(comb_func: partial,
              gather_dim=None) -> Union[Shard, Replicate]:  # TODO @botbw: refactor this
...
def searchShardSingle(tensor: Tensor) -> List[TensorAnn]:
    ...
    _searchShardSingle(set(i for i in range(device_mesh.mesh.dim())), tensor, 0, [])
...
def searchCombSingle(comb_funcs: List[partial]) -> List[Tuple[CombSeq, DeviceAnn]]:
    ans = []
    ...
    _searchCombSingle(set(i for i in range(device_mesh.mesh.dim())), [],
                      [Replicate() for _ in range(device_mesh.mesh.dim())], None)
    return ans
    ...
    for i, args_ann in enumerate(args_anns):
        with open(f"rank{rank}.txt", "w") if LOG else nullcontext(DummyFile()) as f:
            ...
            if all(dim_ann.device_dims is None for ann in tensor_ann for dim_ann in ann):
                ...
                all_dim = [i for i in range(device_mesh.mesh.dim())]
                op_legit_anns_and_comb.append(
                    (args_ann, [partial(identity, group=getRanks(all_dim))],
                     [Replicate() for _ in range(device_mesh.mesh.dim())]))
                continue
...
@torch.no_grad
def test_matmul():
    tensor1 = torch.ones(16, 8, 4) * 3
    tensor2 = torch.ones(16, 4, 8)
    ...
    if rank == 0:
        with open(f'device{tuple(device_mesh.mesh.shape)}-matmul_sol.txt', 'w') as f:
            f.write(f"ans len: {len(ans)}\n")
            for x in ans:
                f.write(f"input spec: {x[0]}, combine funcs: {x[1]}, output spec: {x[2]}\n")


@torch.no_grad
def test_conv():
    layer = torch.nn.Conv2d(32, 16, 4, stride=1)
    layer = broadcast_module(layer)
    tensor = torch.ones(16, 32, 32, 32)
    ...
    ans = searchSolution(torch.conv2d, [tensor, weight, bias, stride, padding, dilation, groups],
                         args_combs)
    if rank == 0:
        with open(f'device{tuple(device_mesh.mesh.shape)}-conv_sol.txt', 'w') as f:
            f.write(f"ans len: {len(ans)}\n")
            for x in ans:
                f.write(f"input spec: {x[0]}, combine funcs: {x[1]}, output spec: {x[2]}\n")
